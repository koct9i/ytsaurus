...
* `enable_table_index_if_has_trivial_mapper` (false) — включить поддержку индексов таблиц и схематизации в случае тривиального mapper-а (когда опция `mapper` не указана).
* `data_weight_per_reduce_job` — целевой объем данных (в байтах) на входе каждого reduce джоба. Также может быть задан как `data_size_per_reduce_job`.
* `compressed_data_size_per_map_job` — ограничение на сжатый размер данных на map джоб.
* `map_locality_timeout` (5 sec) — время, в течение которого планировщик ждет появления свободных ресурсов на конкретных узлах кластера для обеспечения локальности данных map джобов.
* `reduce_locality_timeout` (1 min) — время, в течение которого планировщик ждет появления свободных ресурсов на конкретных узлах кластера для обеспечения локальности данных reduce джобов.
* `max_partition_factor` — максимальный фактор иерархического партицирования. Ограничивает коэффициент ветвления в дереве партиций.
* `input_query` — SQL-подобное выражение для предварительной фильтрации входных данных перед подачей в mapper. Из входных таблиц читаются только те колонки, на которые ссылается запрос. Подробнее в разделе [Настройки операций](../../../../user-guide/data-processing/operations/operations-options.md).
* `input_schema` — схема для выражения `input_query`. Должна указываться только при наличии `input_query`.
...
`Reduce_combiner` также можно принудительно запустить, установив параметр `force_reduce_combiners` в `true`. `Reduce_combiner` получает на вход сортированный поток записей (как обычный reducer). Однако, эти записи не обязательно формируют непрерывный диапазон в порядке `sort_by`. Если некоторые две записи попали в `reduce_combiner` джоб, между ними (в порядке `sort_by`) могут быть записи, которые в этот джоб не попали.

На выход `reduce_combiner` накладывается несколько ограничений:
