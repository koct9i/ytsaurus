...
- `issue_temporary_token` (false) — выдать временный токен для аутентифицированного пользователя, позволяющий получать доступ к хост-кластеру из пользовательских джобов. Значение токена добавляется в `secure_vault` под ключом, заданным опцией `temporary_token_environment_variable_name` (по умолчанию `YT_TOKEN`), и удаляется после завершения операции.
- `temporary_token_environment_variable_name` (`YT_TOKEN`) — имя ключа, под которым временный токен добавляется в `secure_vault`, если включена опция `issue_temporary_token`.
