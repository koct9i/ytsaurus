...
### Важные особенности и corner cases

- Порядок в очереди и оффсеты определены по партициям. Для многотаблетной очереди воспринимайте каждый `partition_index` как независимый поток.
- `register_queue_consumer` поддерживает опциональное поле `partitions`. Если оно задано, регистрация и вычисление lag/тримминга для консьюмера применяются только к указанным партициям.
- Оффсет консьюмера — это индекс первой непрочитанной строки. Если старые строки уже удалены триммингом, чтение с более старого оффсета начнется с текущего `lower_row_index`.
- В `@queue_consumer_partitions` поле `disposition` может быть:
  - `ahead` — оффсет консьюмера больше `upper_row_index` очереди (например, после пересоздания очереди или ручного изменения оффсета).
  - `up_to_date` — непрочитанных строк нет.
  - `pending_consumption` — непрочитанные строки есть и они доступны.
  - `expired` — непрочитанные строки логически есть, но часть уже удалена триммингом и читать их нельзя.
- Автоматический тримминг Queue Agent работает только при двух условиях:
  - у очереди `@auto_trim_config/enable = %true`;
  - у очереди есть хотя бы одна **vital** регистрация.  
  Если vital-консьюмеров нет, Queue Agent тримминг не делает.
- `queue_agent_banned=%true` на очереди или консьюмере отключает ведущую логику Queue Agent для этого объекта (включая тримминг и экспорт для очередей).
- Для replicated и chaos-replicated очередей `static_export_config` нужно задавать на репликах очереди, а не на объекте replicated table.

### Анализ производительности

Для регулярной диагностики используйте статусные атрибуты Queue Agent как источник интроспекции с невысокой частотой (не как high-QPS API):

- Скорость записи очереди:
  - `@queue_status/write_row_count_rate`
  - `@queue_status/write_data_weight_rate`
- База для оценки бэклога по партициям:
  - `@queue_partitions/*/available_row_count`
  - `@queue_partitions/*/available_data_weight`
  - `@queue_partitions/*/commit_idle_time`
- Lag и скорость чтения консьюмера:
...
- Из-за асинхронности обновлений возможны небольшие временные отрицательные значения lag/unread.
- `unread_data_weight` может быть `null` при `ahead` и `expired`.
- Поля веса данных очереди (`trimmed_data_weight`, `available_data_weight`) являются приближенными.
- Для static export значения table lag тоже являются приближением, особенно при cron-подобных расписаниях.

### Queue Agent: краткий how-to для администрирования

1. Инициализируйте (или мигрируйте) state-таблицы Queue Agent в `//sys/queue_agents`:
   ```bash
   python3 yt/python/yt/environment/init_queue_agent_state.py \
       --proxy <cluster_proxy> --root //sys/queue_agents --latest
   ```
2. Проверьте инстансы Queue Agent:
   ```bash
   yt ls //sys/queue_agents/instances
   ```
3. Проверьте состояние компонент на инстансе:
   ```bash
   ...
   ```
4. Временно исключите проблемный инстанс Queue Agent из шардирования:
   ```bash
   ...
   ```
5. Направляйте объекты на нужный stage Queue Agent:
   - Задавайте у очередей/консьюмеров `@queue_agent_stage`, совпадающий со `stage` в конфиге Queue Agent.
   - Для точечного отключения логики Queue Agent у объекта используйте `@queue_agent_banned=%true`.
