...
| --------- | ----------- | -------- |
| `Authorization` | Да (для аутентифицированных запросов) | Учетные данные аутентификации, обычно `OAuth <token>`. |
| `X-YT-Header-Format` | Рекомендуется | Формат YT-специфичных структурированных заголовков (`X-YT-Input-Format`, `X-YT-Output-Format`, `X-YT-Parameters`). |
| `X-YT-Input-Format` | Опционально | Формат входного потока как YSON-строка с атрибутами, закодированная согласно `X-YT-Header-Format`. Перекрывает определение через `Content-Type`. |
| `X-YT-Output-Format` | Опционально | Формат выходного потока как YSON-строка с атрибутами, закодированная согласно `X-YT-Header-Format`. Перекрывает определение через `Accept`. |
| `X-YT-Parameters` | Обычно да | Параметры команды как YSON-словарь, закодированный согласно `X-YT-Header-Format`. |
| `Content-Type` | Опционально | MIME-тип для выбора формата тела запроса (используется при определении входного формата). |
| `Accept` | Опционально | Предпочитаемый(е) MIME-тип(ы) ответа (используется при определении выходного формата). |
| `Content-Encoding` | Опционально | Кодек сжатия для тела запроса. |
| `Accept-Encoding` | Опционально | Допустимый(е) кодек(и) сжатия для тела ответа. |
| `X-YT-Correlation-Id` | Опционально | Идентификатор запроса, задаваемый клиентом, для отладки и корреляции логов. |
| `X-YT-Accept-Framing` | Опционально | Если установить `1`, прокси может вернуть фреймированный ответ (`X-YT-Framing: 1`). |
| `X-YT-User-Name` | Опционально | Логин пользователя для имперсонации. Разрешен только для незабаненных суперпользователей (и пользователей из legacy whitelist). |

### Заголовки и трейлеры ответа { #response_headers }

| Заголовок/трейлер | Где | Описание |
| ----------------- | --- | -------- |
| `X-YT-Request-Id` | Заголовок | Идентификатор запроса, сформированный прокси; используется для поиска запроса в логах. |
| `X-YT-Proxy` | Заголовок | Hostname прокси, обработавшей запрос. |
| `X-YT-Framing` | Заголовок | Равен `1`, если в теле ответа используется фреймированный протокол. |
| `Content-Type` | Заголовок | Фактический MIME-тип ответа, выбранный прокси. |
| `X-YT-Error` | Заголовок или трейлер | Структурированная ошибка {{product-name}} (предпочтительный источник результата команды). |
| `X-YT-Response-Code` | Заголовок или трейлер | Числовой код ответа {{product-name}} (ненулевой означает ошибку на уровне команды). |
| `X-YT-Response-Message` | Заголовок или трейлер | Текстовое диагностическое сообщение об ошибке для `X-YT-Response-Code`. |

Для потоковых ответов (`HTTP 202`) поля, связанные с результатом выполнения команды, обычно передаются в трейлерах.
