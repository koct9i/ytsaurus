...
- `ping_tx` пингует транзакцию на сервере (включая все родительские, если указан `ping_ancestor_transactions`), тем самым обновляя время ее жизни.
...
HTTP API поддерживает данную команду начиная с версии 4. В более ранних версиях используется команда `start_tx`.
...
- Начать новую транзакцию в контексте текущей транзакции.
- Новая транзакция является вложенной (внутренней) по отношению к указанной.
- Это эквивалент команды `start_tx` для API v4 со структурированным выводом.
...
HTTP API поддерживает данную команду начиная с версии 4. В более ранних версиях используется команда `ping_tx`.
...
- Обновить транзакцию, пропинговав ее для продления времени жизни.
- Это эквивалент команды `ping_tx` для API v4 со структурированным выводом.
...
HTTP API поддерживает данную команду начиная с версии 4. В более ранних версиях используется команда `commit_tx`.
...
- Успешно завершить транзакцию.
- Пока есть незавершенные внутренние транзакции, внешняя транзакция не может быть завершена.
- Это эквивалент команды `commit_tx` для API v4 со структурированным выводом.

Параметры:

| **Параметр**               | **Обязательный** | **Значение по умолчанию** | **Описание**                                                 |
| -------------------------- | ------------- | ------------------------- | ------------------------------------------------------------ |
| `transaction_id`             | Да      |                           | Идентификатор транзакции для коммита.                        |
| `ping_ancestor_transactions` | Нет      | `false`                     | Пинговать ли все родительские транзакции в процессе выполнения операции. |
...
HTTP API поддерживает данную команду начиная с версии 4. В более ранних версиях используется команда `abort_tx`.
...
- Прервать транзакцию.
- Все активные внутренние транзакции прерываются.
- Это эквивалент команды `abort_tx` для API v4 со структурированным выводом.

Параметры:

| **Параметр**               | **Обязательный** | **Значение по умолчанию** | **Описание**                                                 |
| -------------------------- | ------------- | ------------------------- | ------------------------------------------------------------ |
| `transaction_id`             | Да      |                           | Идентификатор транзакции для прерывания.                     |
| `ping_ancestor_transactions` | Нет      | `false`                     | Пинговать ли все родительские транзакции в процессе выполнения операции. |
...
- Пингует транзакцию распределенной сессии на сервере (включая все родительские, если указан `ping_ancestor_transactions`), тем самым обновляя время ее жизни.
...
| `operation_id (operation_alias)` | Да      |                           | Идентификатор операции. |
| `job_id`       | Да      |                           | Идентификатор джоба.    |
| `type` | Нет |                            | Тип stderr для получения: `user_job_stderr` или `gpu_check_stderr`. |
...
| `stage` | Нет |                           | Стадия Query Tracker для использования (например, `production`, `testing`). |
...
| `cursor_time` | Нет |                           | Время курсора для пагинации. |
| `cursor_direction` | Нет | `past` | Направление курсора: `past` или `future`. |
| `user` | Нет |                           | Фильтр по пользователю. |
| `state` | Нет |                           | Фильтр по состоянию запроса. |
| `engine` | Нет |                           | Фильтр по движку запросов. |
| `filter` | Нет |                           | Дополнительная строка фильтрации. |
| `limit` | Нет | `100` | Максимальное количество запросов для возврата. |
| `attributes` | Нет |                           | Список атрибутов для получения для каждого запроса. |
| `search_by_token_prefix` | Нет | `false` | Включить поиск по префиксу токена. |
| `use_full_text_search` | Нет | `false` | Включить полнотекстовый поиск. |
| `tutorial_filter` | Нет | `false` | Фильтровать обучающие запросы. |
| `sort_order` | Нет | `cursor` | Порядок сортировки результатов. |
