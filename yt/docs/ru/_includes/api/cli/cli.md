...
Для постоянных настроек используйте общий для Python SDK и CLI конфигурационный файл. По умолчанию это `~/.yt/config` (если файл отсутствует, используется `/etc/ytclient.conf`), путь можно переопределить через `YT_CONFIG_PATH`. Поддерживаются форматы YSON и JSON (`YT_CONFIG_FORMAT`) и выбор профиля (`YT_CONFIG_PROFILE`).

Более подробно о настройках в разделе [Конфигурация клиентской библиотеки](../../../api/python/userdoc.md#configuration).
