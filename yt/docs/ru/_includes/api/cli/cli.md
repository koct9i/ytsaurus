...
Для постоянных настроек используйте общий для Python SDK и CLI конфигурационный файл. По умолчанию это `~/.yt/config`, путь можно переопределить через `YT_CONFIG_PATH`.
