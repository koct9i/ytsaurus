...
1. Из опции `config["token"]`, которая попадает в глобальную конфигурацию из
   переменной окружения `YT_TOKEN`.
2. Из опции `config["token_command"]`, которая запускает локальную команду и
   использует первую строку stdout как токен. Эту опцию также можно
   переопределить переменной окружения `YT_TOKEN_COMMAND` (переменная
   окружения действует только на конфигурацию глобального клиента, детальней
   написано в [разделе](#configuration)). Команда запускается без shell, а
   таймаут ее выполнения задается опцией `config["token_command_timeout"]` в
   миллисекундах (по умолчанию 10 секунд).
3. Из файла, указанного в `config["token_path"]`; по умолчанию эта опция
   равна `~/.yt/token`, также опция может быть переопределена с помощью
   переменной окружения `YT_TOKEN_PATH` (переменная окружения действует только
   на конфигурацию глобального клиента, детальней написано в
...
У каждой схемы есть соответствующий ей идентификатор. Его можно посмотреть на таблице с помощью команды:
```python
schema = TableSchema.from_yson_type(yt.get("//path/to/table/@schema_id"))
```
Также по ID можно получить материализованную схему:
```python
schema = TableSchema.from_yson_type(yt.get("#<SCHEMA_ID>/@value"))
```

Важно отметить, что между схемами и их ID отношение один ко многим, а не один к одному. На двух таблицах с одинаковой схемой у схемы могут быть разные идентификаторы в том случае, если таблицы имеют разные нативные селлы. Дополнительно стоит упомянуть, что время жизни схем зависит от времени жизни таблиц и чанков, ссылающихся на эту схему. То есть если все таблицы, которые используют схему, будут удалены, то будет удалена и схема. При создании новой таблицы с ранее удалённой схемой, у схемы поменяется ID.

Основной сценарий использования ID - дедупликация схем, поскольку объекты схем могут быть достаточно тяжёлыми. Дополнительно стоит отметить, что работа со идентификаторами схем поддержана в глаголах `create` и `alter`.
...
Как добавить конфиг на кластер:

- создать узел `default` типа **document** в директории `//sys/client_config`  (`//sys/client_config/default`)
- добавить нужные параметры в документ (см поддерживаемые в `default_config.py` — они инициализированы через `RemotePatchable`)
- проверить применение конфига можно в дебаг логах клиента (`YT_LOG_LEVEL=DEBUG`) и через cli — `show-default-config`
