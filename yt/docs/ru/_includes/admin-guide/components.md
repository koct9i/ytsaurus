...
Инстансы YQL Agent оркестрируют исполнение запросов [YQL](../../yql/index.md) в {{product-name}}. Каждый инстанс получает YQL-запрос, компилирует его с помощью оптимизатора YQL и преобразует в цепочку Map/Reduce-операций {{product-name}}, которые передаются планировщику кластера. YQL Agent управляет планированием запроса, отслеживает прогресс исполнения и собирает результаты. Несколько инстансов YQL Agent могут работать одновременно для обеспечения параллелизма и отказоустойчивости.
...
CHYT (ClickHouse over {{product-name}}) — технология, запускающая кластер серверов [ClickHouse](https://clickhouse.com/) непосредственно на вычислительных узлах {{product-name}} через Vanilla-операцию. Такая операция называется *кликой*. CHYT позволяет выполнять быстрые аналитические SQL-запросы напрямую по таблицам {{product-name}} без копирования данных в отдельную инсталляцию ClickHouse. Управление кликами осуществляется через [Strawberry Controller](#strawberry-controller). Подробнее см. в [документации по CHYT](../../user-guide/data-processing/chyt/about-chyt.md).

### SPYT

SPYT (Spark over {{product-name}}) — технология, запускающая кластер [Apache Spark](https://spark.apache.org/) на вычислительных узлах {{product-name}} через Vanilla-операцию. SPYT позволяет запускать Spark-джобы, которые читают данные из таблиц {{product-name}} и записывают в них результаты, используя всю мощь экосистемы Spark: DataFrame, Spark SQL, Spark Streaming и MLlib. Управление Spark-кластерами осуществляется через [Strawberry Controller](#strawberry-controller). Подробнее см. в [документации по SPYT](../../user-guide/data-processing/spyt/overview.md).
