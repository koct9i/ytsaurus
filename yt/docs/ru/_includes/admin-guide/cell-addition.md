...
2. Можно заранее подготовить @cluster_connection с новыми селлами (про @cluster_connection написано ниже в пункте 16).
...
Если не сделать этого, то новым мастерным селлам автоматически назначатся роли `chunk_host` и `cypress_node_host` (детальнее про существующие роли можно прочитать в пункте 14), и последующее изменение ролей может привести к потере данных.
...
### 13. Ещё раз построить снапшот с read-only, перезагрузить мастера и выйти из read-only
```bash
yt-admin build-master-snapshots —read-only —wait-for-snapshot-completion
```

После регистрации новых селлов и репликации глобальных объектов необходимо зафиксировать это состояние в снапшоте, перезагрузить мастера и выйти из read-only режима (повторить шаги 7–10).

### 14. Прописать новым селлам роли
...
### 15. Поднять ноды (на версию с новым конфигом)
...
### 16. Дописать новые селлы в cluster connection
...
### 17. Включить chunk refresh и chunk requisition update
...
```
### 18. Дождаться, пока все ноды зарегистрируются и для чанков сойдется refresh
В зависимости от количества нод и чанков на кластере этот шаг может занимать существенное время.

### 19. Поднять остальные компоненты
...
### 20. Дождаться перехода кластера в режим штатной работы
