...
Периодически выпускаются новые релизы, а также вносятся исправления в рамках текущих стабильных релизов.{% if audience == "public" %} Почитать подробнее про выпущенные релизы можно [в отдельном разделе](../../admin-guide/releases.md).{% endif %} Докер-образы со стабильными релизами имеют вид `ghcr.io/ytsaurus/ytsaurus:23.N.M`, например `ghcr.io/ytsaurus/ytsaurus:{{yt-server-version}}`.
...
Перед обновлением {{product-name}} необходимо обновить оператор на последний релиз. Работоспособность новых стабильных образов не гарантируется со старым оператором, а также не гарантируется работоспособность оператора с нестабильными версиями {{product-name}}, например с `dev`.{% if audience == "public" %} Найти список стабильных образов можно на страницe [Релизы](../../admin-guide/releases).{% endif %}
...
Каким компонентам действительно разрешено обновляться, определяет поле `updatePlan`.

### План обновления

Поле `updatePlan` в спецификации `Ytsaurus` заменяет устаревший флаг `enableFullUpdate`. Оно определяет, какие компоненты оператору разрешено обновлять.

Если `updatePlan` пустой, оператор блокирует обновление. Чтобы обновить весь кластер, разрешите класс `Everything`:
...
Также с помощью `updatePlan` можно обновлять только часть кластера:

- `class: Stateless` обновляет только stateless-компоненты.
- `class: Nothing` явно блокирует обновление всех компонент.
- `component` выбирает конкретный тип компоненты и, при необходимости, группу инстансов.
- `concurrency` ограничивает количество выбранных групп инстансов, обновляемых одновременно.

В одном плане можно комбинировать несколько селекторов. Например, сначала обновить мастера, а затем отдельным изменением разрешить более широкое обновление stateless-компонент.

Если `updatePlan` разрешает обновлять только часть компонент, оператор обновит их, а остальные изменения оставит заблокированными. В этом случае кластер может перейти в состояние `UpdateBlocked`, пока вы не расширите план или не откатите изменения спецификации.

### Image heater

Image heater заранее скачивает необходимые образы на выбранные Kubernetes-узлы перед инициализацией кластера и перед обновлениями. Это уменьшает время ожидания при старте подов, когда им нужно загрузить новые образы.

Чтобы включить его для кластера, задайте `clusterFeatures.enableImageHeater`:
...
Оператор также может запускать image heater вместе с `updatePlan` для компонент, которые участвуют в обновлении.

### Rolling update

В `updatePlan` можно задавать и стратегию обновления для выбранных компонент. Для компонент на базе StatefulSet оператор поддерживает стратегию `rollingUpdate`:
...
При `rollingUpdate` поды пересоздаются автоматически в соответствии со стратегией rolling update у StatefulSet. Бюджет недоступных подов вычисляется из `minReadyInstanceCount` компоненты, поэтому оператор сохраняет требуемое количество готовых инстансов в процессе обновления.

Также можно использовать `strategy.runPreChecks: true`, чтобы перед началом rollout запускались предварительные проверки для выбранных компонент.
...
Если обновление окажется [невозможным](#impossible), то кластер останется в состоянии `Updating`, а статус обновления станет `ImpossibleToStart`. В таком случае можно откатить оператор, тогда обновление отменится и кластер перейдёт в состояние `Running`. Либо можно временно запретить обновления в спецификации `Ytsaurus`:
...
Это отменит текущее обновление и не даст новому оператору запустить следующее. После этого можно вернуть кластер в рабочее состояние и затем снова расширить план. Для полного обновления кластера используйте `class: Everything`.
