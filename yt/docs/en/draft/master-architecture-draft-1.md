...
A snapshot request can be canceled when the peer leaves its Hydra epoch, for example because an election changes the leader or the peer is restarted: stopping the epoch cancels the outstanding snapshot promise with `Hydra peer has stopped`. In normal mode this does not change the replicated automaton state or stop writes; after the new leader becomes active, an operator or the periodic trigger can request another snapshot. Read-only mode is different because `EnterReadOnly` is committed before snapshot construction starts. Cancellation does not roll that mutation back, and Hydra deliberately refuses to start a replacement snapshot while already read-only. If no valid snapshot was completed, subsequent requests fail with `ReadOnlySnapshotBuildFailed`; the operator must diagnose the original failure and then either explicitly exit read-only mode and repeat the quiescing snapshot procedure, or restore/recover the cell by the incident-specific procedure. Do not resume writes merely to clear the error unless it is safe for the maintenance operation.
