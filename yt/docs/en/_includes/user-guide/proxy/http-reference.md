...
| ------ | --------- | ----------- |
| `Authorization` | Yes (for authenticated requests) | Authentication credentials, typically `OAuth <token>`. |
| `X-YT-Header-Format` | Recommended | Format of YT-specific structured headers (`X-YT-Input-Format`, `X-YT-Output-Format`, `X-YT-Parameters`). |
| `X-YT-Input-Format` | Optional | Input stream format as a YSON string with attributes encoded according to `X-YT-Header-Format`. Overrides `Content-Type` mapping. |
| `X-YT-Output-Format` | Optional | Requested output stream format as a YSON string with attributes encoded according to `X-YT-Header-Format`. Overrides `Accept` mapping. |
| `X-YT-Parameters` | Usually yes | Command parameters as a YSON dict encoded according to `X-YT-Header-Format`. |
| `Content-Type` | Optional | MIME type for request body format selection (used for input format inference). |
| `Accept` | Optional | Preferred MIME type(s) for response body format selection (used for output format inference). |
| `Content-Encoding` | Optional | Compression codec for request body. |
| `Accept-Encoding` | Optional | Accepted compression codec(s) for response body. |
| `X-YT-Correlation-Id` | Optional | Client-generated request id for debugging and log correlation. |
| `X-YT-Accept-Framing` | Optional | If set to `1`, allows proxy to return framed response (`X-YT-Framing: 1`). |
| `X-YT-User-Name` | Optional | Impersonated user login. Allowed only for unbanned superusers (and legacy whitelist users). |

### Response headers and trailers { #response_headers }

| Header/Trailer | Where | Description |
| -------------- | ----- | ----------- |
| `X-YT-Request-Id` | Header | Request id generated by proxy; use it to locate request logs. |
| `X-YT-Proxy` | Header | Hostname of the proxy that handled the request. |
| `X-YT-Framing` | Header | Set to `1` when framed protocol is used in the response body. |
| `Content-Type` | Header | Actual response MIME type selected by proxy. |
| `X-YT-Error` | Header or trailer | Structured {{product-name}} error (preferred source of command result). |
| `X-YT-Response-Code` | Header or trailer | Numeric {{product-name}} response code (non-zero means command-level error). |
| `X-YT-Response-Message` | Header or trailer | Human-readable/diagnostic error message corresponding to `X-YT-Response-Code`. |

For streaming responses (`HTTP 202`), result-related fields are typically sent in trailer headers.
