...
- `issue_temporary_token` (false) — issues a temporary token for the authenticated user to access the host cluster from user jobs. The token value is added to `secure_vault` under the key specified by `temporary_token_environment_variable_name` (defaults to `YT_TOKEN`) and is removed after the operation is finished.
