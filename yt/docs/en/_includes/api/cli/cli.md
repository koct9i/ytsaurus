...
For persistent settings, use the shared Python SDK/CLI config file. By default, it is `~/.yt/config` (if this file is missing, `/etc/ytclient.conf` is used), and you can override the path via `YT_CONFIG_PATH`. The file supports YSON and JSON formats (`YT_CONFIG_FORMAT`) and profile selection (`YT_CONFIG_PROFILE`).
